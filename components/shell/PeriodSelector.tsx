...
export function PeriodSelector({ options }: { options: PeriodOptionDTO[] }) {
  ...
  return (
    <label className="flex h-[38px] items-center gap-2 rounded-[10px] border border-line bg-app px-3 max-[960px]:hidden">
      <CalendarDays size={15} strokeWidth={1.8} className="text-ink-dim" aria-hidden />
      <span className="sr-only">Período</span>
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="cursor-pointer border-none bg-transparent text-[13px] font-semibold text-ink outline-none"
      >
        {options.map((o) => (
